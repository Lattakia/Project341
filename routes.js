...
var appObj = {
            ...
            collection.find({
                ...
                if (accounttypeuser == "student") {
                    function renderThis(dataArray) {
                        res.render('survey_selection.ejs', {
                            data: dataArray
                        });
                    }
    ...
    displaySurveyResults: function (req, res) {
        ...
        MongoClient.connect(URLMain, function (err, db) {
            ...
            collection.find({
                'local.username': profSurveyMaker
                ...
                MongoClient.connect(URLSurveyDataBase, function (err, db) {
                    ...
                    collectionSurvey.find({
                        surveyMakerName: profSurveyMakerName
                    }).toArray(function (err, docs) {
                        if (err) return;
                        // Send the documents from the database collection to the client to process.
                        res.render('survey-results.ejs', {
                            dbSurveyDocs: docs
                        });
                        db.close();
                    });
        ...
        MongoClient.connect(URLMain, function (err, db) {
            ...
            collection.find({
                ...
                if (accounttypeuser == "student") {
                    ...
                    collection.find({
                        'local.firstname': profFirstName,
                        'local.lastname': profLastName
                    }).toArray(function (err, docs) {
                        if (err) return;
                        surveyMaker = docs[0];
                        MongoClient.connect(URLSurveyDataBase, function (err, db) {
                            ...
                            collectionSurvey.find({
                                Name: surveyMaker['local']['username']
                            }).toArray(function (err, docs) {
                                if (err) return;
                                // Send the documents from the database collection to the client to process.
                                if (docs.length == 0) {
                                    res.render('no-survey.ejs', {
                                        surveyMakerName: profName
                                    });
                                } else {
            ...
            MongoClient.connect(URLSurveyDataBase, function (err, db) {
                ...
                collection.find({
                    ...
                    res.render('ss-results', {
                        oldResults: JSON.stringify(oldSurveyResults),
                        insertedResults: JSON.stringify(newSurveyResultsUpdated),
                        updatedResults: JSON.stringify(newSurveyResultsUpdated)
                    });
                });
        ...
        MongoClient.connect(URLMyDataBase, function (err, db) {
            ...
            if (req.query.title != "") {
                ...
                collection.insert({
                    ...
                    posted: req.query.mylittletextbox,
                    tags: req.query.tags,
                    username: req.session.session.title,
                    date: thedate
                }, function (err, result) {
                    db.close();
                });
                    ...
                    if (user) {
                        ...
                        if (user.local.accounttype == 'student') {
                            ...
                            studentProfileModel.findOne({
                                ...
                                res.render('profileStudentEdit', {
                                    userInfo: userObject,
                                    profileInfo: sProfileObject
                                });
                            ...
                            teacherProfileModel.findOne({
                                ...
                                //Pass data to prepopulate form
                                res.render('profileTeacherEdit', {
                                    userInfo: userObject,
                                    profileInfo: tProfileObject
                                });
                            });
                        }
